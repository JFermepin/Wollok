...
class Nivel {
	...
	var property mejoras = [new Arma(nombre = "escopeta", sonidoRecarga = "escopeta-recarga.mp3", sonidoDisparo = "escopeta-tiro.mp3", danio = 12, tiempoRecarga = 4500, sonidoRecoger = "escopeta-recoger.mp3", posicion = null, cargador = 8),
		new Arma(nombre = "rifle", sonidoRecarga = "rifle-recarga.mp3", sonidoDisparo = "rifle-tiro.mp3", sonidoRecoger = "rifle-recoger.mp3", danio = 8, tiempoRecarga = 2100, posicion = null, cargador = 20)]
	...
	method agregarVisuales(){
		...
		game.addVisual(new ItemDelHud(imagen = numeroAString.conversion(objetivo) + ".png", posicion = game.at(16,1)))
		
		if(soldado.arma().balas() == 0){
			game.addVisual(new ItemDelHud(imagen = "recarga.png", posicion = game.at(22,1)))
			soldado.recarga()
		}else{
			game.addVisual(new ItemDelHud(imagen = numeroAString.conversion(soldado.arma().balas()) + ".png", posicion = game.at(22,1)))	
		}
		...
		game.onTick(2000,"sonidoZombie",{
			...
			if((probabilidadesDeSonido >= 1) && (probabilidadesDeSonido <= 8)){
				game.sound("zombie" + numeroAString.conversion(probabilidadesDeSonido) + ".mp3").play()
			}
		})
	}
	
	method mostrar(){
		maxZombies = dificultad * 5
	...
	method siguienteMejora(){
		if(!mejoras.isEmpty()){
			var probabilidadesDeMejora = 0.randomUpTo(dificultad).truncate(0)
	...
	method resetearNivel(){
		...
		mejoras = [new Arma(nombre = "escopeta", sonidoRecarga = "escopeta-recarga.mp3", sonidoDisparo = "escopeta-tiro.mp3", danio = 12, tiempoRecarga = 4500, sonidoRecoger = "escopeta-recoger.mp3", posicion = null, cargador = 8),
		new Arma(nombre = "rifle", sonidoRecarga = "rifle-recarga.mp3", sonidoDisparo = "rifle-tiro.mp3", danio = 8, tiempoRecarga = 2100, sonidoRecoger = "rifle-recoger.mp3", posicion = null, cargador = 20)]
		if(dificultad == 1){
			objetivo = 20
		}
		if(dificultad == 2){
